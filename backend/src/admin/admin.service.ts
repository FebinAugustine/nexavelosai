...
export class AdminService {
  ...
  async findAllUsers(): Promise<UserDocument[]> {
    return this.userModel.find().select('-password -verificationCode -resetPasswordToken').exec();
  }

  async findUserById(id: string): Promise<UserDocument> {
    const user = await this.userModel.findById(id).select('-password -verificationCode -resetPasswordToken').exec();
    if (!user) {
  ...
  async createUser(createUserByAdminDto: CreateUserByAdminDto): Promise<UserDocument> {
    const existingUser = await this.userModel.findOne({ email: createUserByAdminDto.email }).exec();
    if (existingUser) {
  ...
  async updateUser(id: string, updateUserByAdminDto: UpdateUserByAdminDto): Promise<UserDocument> {
    ...
    const user = await this.userModel.findByIdAndUpdate(id, update, { new: true }).select('-password -verificationCode -resetPasswordToken').exec();
    if (!user) {
      throw new NotFoundException(`User with ID ${id} not found.`);
    }
    return user;
